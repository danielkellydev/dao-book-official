...
const verifyPractitionerOwnership = asyncHandler(async (req, res, next) => {
  ...
  const objectPractitionerId =
    req.body.practitionerId || req.params.practitionerId;
...
const verifyOwnership = (Model, idFieldName) =>
  asyncHandler(async (req, res, next) => {
    ...
    try {
      ...
      const object =
        (await Model.findById(objectId)) ||
        (await Model.findOne({ _id: objectId })) ||
        (await Model.findOne({ [idFieldName]: objectId }));
      if (!object) {
        return res.status(404).json({
      ...
      const loggedPractitionerId = req.practitioner
      ...
      if (
        (loggedPractitionerId &&
          loggedPractitionerId.toString() ===
            objectPractitionerId.toString()) ||
        (loggedPatientId &&
          loggedPatientId.toString() === objectPatientId.toString())
      ) {
